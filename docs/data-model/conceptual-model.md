...
| Entidad canónica | Propósito y owner | Identidad, claves/códigos y FD conocidas | Ciclo de vida, normalización e invariantes | Primera etapa / referencias |
|---|---|---|---|---|
| `Establishment` | Contexto territorial operativo. Owner: Agro Ops. | UUID interno. Código/nombre no tienen unicidad conocida. FD: UUID → identidad/contexto vigente. | Mutable con historia si cambia alcance espacial; BCNF objetivo. | Etapa 3. Mapeable, no subordinado, a Finnegans. |
| `BasePlot` | Identidad territorial estable subyacente. Owner: Agro Ops. | UUID interno y posible código canónico estable, cuya unicidad es Unknown. FD: UUID → establecimiento e identidad; geometría requiere historia independiente. | History-preserving/versioned. Un cambio físico real no reescribe campañas pasadas. BCNF objetivo para identidad; geometría temporal se diseñará aparte. | Etapa 3. Plan Etapa 3; ADR-003. |
| `Campaign` | Contexto temporal/productivo. Owner: Agro Ops. | UUID interno. Año/nombre/rango no son clave candidata conocida. FD: UUID → período/estado. | History-preserving; cerrar o corregir no altera estructura previa. BCNF objetivo. | Etapa 3. |
| `OperationalUnit` | Geografía operativa específica de una campaña. Owner: Agro Ops. | UUID interno. Código legible y su scope son Unknown. FD conocida: UUID → una Campaign y la identidad estable de la UOP; las dependencias de sus versiones geométricas son Unknown. | History-preserving/versioned; no se reutiliza entre campañas. BCNF objetivo, sujeto a resolver vínculo con `BasePlot`. | Etapa 3. Polygon/MultiPolygon según ADR-003. |
...
| Campaign ↔ Establishment | Alcance territorial de una campaña. | Cardinalidad y obligatoriedad: **Unknown**. | **Unresolved:** no se infiere cadena ni identidad de una posible participación. Afecta keys de Campaign/UOP; resolver Etapa 3. |
...
| BasePlot ↔ OperationalUnit | Territorio base que explica la UOP. | Máximos y obligatoriedad exactos: **Unknown**; no se decide si es 1:N o M:N. | **Unresolved:** depende de la cardinalidad y de si existe participación con área/proporción/origen. Toda versión histórica permanece; resolver Etapa 3. |
...
- `Establishment` delimita contexto operacional; ADR-003 permite `Polygon` o `MultiPolygon` según corresponda.
- `BasePlot` aporta identidad estable y geometría territorial base (`Polygon`/`MultiPolygon`). Su geometría puede versionarse ante cambios físicos reales.
- `Campaign` aporta contexto temporal/productivo; no se inserta como eslabón físico ficticio entre BasePlot y UOP.
- `OperationalUnit` es geografía de una campaña (`Polygon`/`MultiPolygon`) y puede variar entre campañas.
- `OperationalUnitUse` conserva actividad/cultivo/uso y vigencia.

Todas las geometrías territoriales usan un SRID explícito y consistente; el SRID concreto y estrategia de proyección se resuelven en Etapa 3. Las superficies derivadas deben ser reproducibles. Una geometría inválida no puede confirmarse como territorio operativo: debe rechazarse o quedar en estado pendiente de corrección con diagnóstico, sin “arreglo” silencioso.

La geometría UOP confirmada debe estar dentro del alcance territorial documentado del establecimiento y de sus bases. La regla exacta de contención por `BasePlot`, solapamiento entre UOP, tolerancias y excepciones no está determinada. La adyacencia no se modela porque ningún requisito actual la necesita.

Invariante esencial: campaña N+1 puede dividir o unir el territorio de modo diferente sin modificar geometría, asignación ni explicación de campaña N. No se decide todavía si una UOP puede cubrir múltiples BasePlot o un BasePlot participar en varias UOP de una campaña; esa decisión cambia la asociación, constraints de área y posibles dependencias multivaluadas y debe cerrarse en Etapa 3.
...
| Campaign | UUID; business key adicional Unknown | UUID → período/estado; scope territorial Unknown | Unknown / not yet assessable; política: ≥3NF | Target BCNF; compliance not yet assessable | Ninguna MVD demostrada | Ninguna JD demostrada; revisar ternaria sólo si se prueba | Resolver alcance territorial. |
| OperationalUnit | UUID; código natural adicional Unknown | UUID → Campaign y representación vigente; dependencias de geometría/version Unknown | Unknown / not yet assessable; política: ≥3NF | Target BCNF; compliance not yet assessable | Afecta candidate keys / FD; 4NF debe analizarse sólo si una futura relación candidata combina dependencias multivaluadas genuinas e independientes de participaciones BasePlot y usos | Ninguna JD demostrada; revisar ternaria sólo si la semántica lo exige | Resolver cardinalidades Etapa 3. |
...
| ¿Campaign es global a Organization, por Establishment o M:N con establecimientos? | Evita imponer una cadena territorial falsa. | Relaciones Campaign–Establishment–UOP y claves. | Etapa 3. |
| ¿Una UOP puede abarcar varios BasePlot y un BasePlot participar en varias UOP dentro de una Campaign? | Cambia 1:N vs M:N, áreas, contención y posible asociación con atributos; 4NF sólo entra en juego si una relación candidata mezcla dependencias multivaluadas independientes. | Diseño UOP–BasePlot y constraints espaciales. | Etapa 3. |
| ¿Cuáles son reglas/tolerancias de solapamiento, contención y excepciones? | Determinan validez geográfica sin destruir historia. | Invariantes espaciales y estado de confirmación. | Etapa 3. |
