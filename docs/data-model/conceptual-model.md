...
| `ActivityUse` | Vocabulario de clasificación territorial de una Organization. Owner: Agro Ops. | UUID interno y código canónico único por Organization. FD: UUID → Organization, código y significado. | Maestro con baja lógica; su identidad y significado no se reescriben. Una baja no invalida usos históricos. BCNF. No contiene semántica agronómica. | Etapa 3.3. |
| `OperationalUnitUse` | Asociación temporal que asigna un `ActivityUse` a una UOP. Owner: Agro Ops. | UUID interno, clave candidata surrogate. FD: UUID → UOP, uso y vigencia. Organization y Campaign se derivan de la UOP. | History-preserving. Vigencia civil semiabierta `[inicio, fin)`, no vacía y contenida en la Campaign. Una UOP admite cero o más usos secuenciales, nunca simultáneos; permite límites contiguos y brechas. BCNF. | Etapa 3.3. |
...
| OperationalUnit ↔ ActivityUse | Clasificación territorial asignada. | UOP `0..*` ↔ Use `0..*`; cada UOP puede tener cero o más asociaciones secuenciales, y cada Use puede aplicarse a varias UOP de su Organization. | Identificante para `OperationalUnitUse`: el uso asignado no existe sin UOP y ActivityUse. La asociación tiene vigencia semiabierta `[inicio, fin)`; no sobrescribe historia ni permite usos simultáneos en una UOP. |
...
- `OperationalUnitUse` conserva la clasificación territorial mediante una asociación temporal; no existe un `current_use_id` mutable en la UOP.
...
La semántica de uso territorial queda cerrada: una UOP puede no tener uso asignado y puede tener usos secuenciales durante su Campaign; los intervalos contiguos son válidos y también lo son las brechas. Una misma UOP no admite usos simultáneos. Si el terreno tiene usos simultáneos distintos, se representa mediante UOP con geometrías distintas. La asociación temporal se preserva históricamente, incluso cuando el `ActivityUse` se desactiva. Intercultivos, consociaciones y demás semántica agronómica siguen siendo responsabilidad de Agriculture en Etapa 5 y no debilitan este invariante territorial.
...
| OperationalUnitUse | UUID; no se adopta natural frágil de UOP/uso/fechas | UUID → UOP/use/vigencia; UOP → Campaign/Organization | BCNF para atributos materializados; asociación temporal separada | Sí | La prohibición de simultaneidad se expresa por rango, no como colección independiente | Ninguna JD no trivial demostrada | Vigencia semiabierta y no solapamiento cerrados en Etapa 3.3. |
