...
#[openapi(
    ...
    paths(health, ready, version, internal_worker_status),
    components(schemas(StatusResponse, VersionResponse, ServiceStatus, ServiceStatusReport)),
...
mod tests {
    ...
    async fn openapi_document_exposes_current_api_contract_and_request_identity_headers() {
        ...
        for path in ["/health", "/ready", "/version", "/internal/worker/status"] {
